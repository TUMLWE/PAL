...
The present framework adopts a structured three-layer approach. These are named *ITFC*, *HOST*, and *SUBMODEL*. Each layer cantains specific applications, each designed to serve a distinct purpose:
...
When a new project is created using the :ref:`Graphical User Interface (GUI)<user_guide_GUI>`, a folder is automatically generated with the project's name. Two additional folders, **ReferenceCFiles** and **SimulinkModels**, are created to store the reference PLC source code and necessary *Simulink* models for the *SUBMODEL* apps.
