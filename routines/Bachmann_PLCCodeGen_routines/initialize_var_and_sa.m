function out_txt = initialize_var_and_sa(ReadInputVarFromSVI, VarNamePLC, InputType, Connect, OutputPortname, OutputType)


string_warning = ['Variable Type to be included in App not yet implemented'];

exchanged_Varname_List = [VarNamePLC(ReadInputVarFromSVI); OutputPortname(Connect)]; 
exchanged_Vartype_List = [ InputType(ReadInputVarFromSVI); OutputType(Connect)];

Field2Text = [newline newline];

for iVar = 1 : length(exchanged_Varname_List)

    varname = exchanged_Varname_List{iVar};
    vartype = exchanged_Vartype_List{iVar};
    
    % define SVIAddress
    String = ['MLOCAL SVI_ADDR SA_' varname ';' newline ...
                ];

    % define and initialize exchanged variables
    
    switch vartype 
        case 'STRING'

            warning(string_warning)
            keyboard

        case 'REAL32'

            String = [String 'MLOCAL REAL32 ' varname ' = 0.0f;' newline];

        case 'BOOL8'

            String = [String 'MLOCAL BOOL8 ' varname ' = 0;' newline];

        case 'SINT16'

            warning(string_warning)
            keyboard

        otherwise
            
            warning(string_warning)
            keyboard
            
    end

        Field2Text = [Field2Text String];

end

out_txt = Field2Text;
end

%% Old and outdated version

% % % % % % String = TXTSource(k1:k2);
% % % % % 
% % % % % % ArrayPLC = ReadVarFromSVI;
% % % % % VariablesArrayInHOST = {};
% % % % % % idxArray = find(ArrayPLC);
% % % % % Field2Text = [newline newline];
% % % % % iVV = 1;
% % % % % 
% % % % % for iVar = 1 : length(ReadInputVarFromSVI)
% % % % % 
% % % % %     if ReadInputVarFromSVI(iVar)
% % % % %         VariablesArrayInHOST{iVV} = VarNamePLC{iVar};
% % % % % 
% % % % %         if strcmp(InputType{iVar}, 'STRING')
% % % % % 
% % % % %             %     String = ['MLOCAL SVI_ADDR SA_' VariablesArrayInENOHOST{iVV} ';' newline ...
% % % % %             %         'MLOCAL CHAR ' VariablesArrayInENOHOST{iVV} '[' num2str(NElem) '];' newline newline];
% % % % %             %
% % % % % 
% % % % %             %     String = ['MLOCAL SVI_ADDR SA_' VariablesArrayInHOST{iVV} ';' newline ...
% % % % %             %         ];
% % % % % 
% % % % %             fprintf('Variable Type to be included in App not yet implemented')
% % % % %             keyboard
% % % % % 
% % % % %         elseif strcmp(InputType{iVar}, 'REAL32')
% % % % % 
% % % % %             %     String = ['MLOCAL SVI_ADDR SA_' VariablesArrayInENOHOST{iVV} ';' newline ...
% % % % %             %         'MLOCAL ' Type{IdxVar} ' ' VariablesArrayInENOHOST{iVV} ';' newline newline];
% % % % %             %
% % % % %             String = ['MLOCAL SVI_ADDR SA_' VariablesArrayInHOST{iVV} ';' newline ...
% % % % %                 ];
% % % % %             String = [String 'MLOCAL REAL32 ' VariablesArrayInHOST{iVV} ' = 0.0f;' newline];
% % % % % 
% % % % % 
% % % % %         elseif strcmp(InputType{iVar}, 'BOOL8')
% % % % % 
% % % % %             String = ['MLOCAL SVI_ADDR SA_' VariablesArrayInHOST{iVV} ';' newline ...
% % % % %                 ];
% % % % %             String = [String 'MLOCAL BOOL8 ' VariablesArrayInHOST{iVV} ' = 0;' newline];
% % % % % 
% % % % %         elseif strcmp(InputType{iVar}, 'SINT16')
% % % % % 
% % % % %             fprintf('Variable Type to be included in App not yet implemented')
% % % % %             keyboard
% % % % % 
% % % % %         else
% % % % % 
% % % % %             fprintf('Variable Type to be included in App not yet implemented')
% % % % %             keyboard
% % % % % 
% % % % %         end
% % % % % 
% % % % %         Field2Text = [Field2Text String];
% % % % % 
% % % % %         iVV = iVV + 1;
% % % % % 
% % % % %     end
% % % % % 
% % % % % end
% % % % % 
% % % % % 
% % % % % VariablesArrayInHOST = {};
% % % % % % idxArray = find(ArrayPLC);
% % % % % iVV = 1;
% % % % % 
% % % % % for iVar = 1 : length(Connect)
% % % % % 
% % % % %     if Connect(iVar)
% % % % %         VariablesArrayInHOST{iVV} = OutputPortname{iVar};
% % % % % 
% % % % %         if strcmp(OutputType{iVar}, 'STRING')
% % % % % 
% % % % %             %     String = ['MLOCAL SVI_ADDR SA_' VariablesArrayInENOHOST{iVV} ';' newline ...
% % % % %             %         'MLOCAL CHAR ' VariablesArrayInENOHOST{iVV} '[' num2str(NElem) '];' newline newline];
% % % % %             %
% % % % % 
% % % % %             %     String = ['MLOCAL SVI_ADDR SA_' VariablesArrayInHOST{iVV} ';' newline ...
% % % % %             %         ];
% % % % % 
% % % % % 
% % % % %             fprintf('Variable Type to be included in App not yet implemented')
% % % % %             keyboard
% % % % % 
% % % % %         elseif strcmp(OutputType{iVar}, 'REAL32')
% % % % % 
% % % % %             %     String = ['MLOCAL SVI_ADDR SA_' VariablesArrayInENOHOST{iVV} ';' newline ...
% % % % %             %         'MLOCAL ' Type{IdxVar} ' ' VariablesArrayInENOHOST{iVV} ';' newline newline];
% % % % %             %
% % % % %             String = ['MLOCAL SVI_ADDR SA_' VariablesArrayInHOST{iVV} ';' newline ...
% % % % %                 ];
% % % % %             String = [String 'MLOCAL REAL32 ' VariablesArrayInHOST{iVV} ' = 0.0f;' newline];
% % % % % 
% % % % %         elseif strcmp(OutputType{iVar}, 'SINT16') || strcmp(OutputType{iVar}, 'BOOL8')
% % % % % 
% % % % %             fprintf('Variable Type to be included in App not yet implemented')
% % % % %             keyboard
% % % % % 
% % % % %         else
% % % % % 
% % % % %             fprintf('Variable Type to be included in App not yet implemented')
% % % % %             keyboard
% % % % % 
% % % % %         end
% % % % % 
% % % % %         Field2Text = [Field2Text String];
% % % % % 
% % % % %         iVV = iVV + 1;
% % % % % 
% % % % %     end
% % % % % end
