...
/* Project includes */
#include "ifcscada.h"
#include "ifcscada_e.h"
#include "ifcscada_int.h"

/* Functions: administration, to be called from outside this file */
SINT32  ifcscada_AppEOI(VOID);
VOID    ifcscada_AppDeinit(VOID);
SINT32  ifcscada_CfgRead(VOID);
SINT32  ifcscada_SviSrvInit(VOID);
VOID    ifcscada_SviSrvDeinit(VOID);

/* Functions: administration, to be called only from within this file */
MLOCAL VOID ifcscada_CfgInit(VOID);
...
IFCSCADA_BASE_PARMS ifcscada_BaseParams;
...
MLOCAL TASK_PROPERTIES TaskProperties_aControl = {
    "aIFCSCADA_Ctrl",                 /* unique task name, maximum length 14 */
...
MLOCAL SVI_GLOBVAR SviGlobVarList[] = {
    {"CycleCounter", SVI_F_INOUT | SVI_F_UINT32, sizeof(UINT32), (UINT32 *) & CycleCount, 0, NULL,
     NULL}
    ,
    {"ModuleVersion", SVI_F_OUT | SVI_F_STRING, sizeof(ifcscada_Version),
     (UINT32 *) ifcscada_Version, 0, NULL, NULL}
};
...
SINT32 ifcscada_AppEOI(VOID)
{
    ...
    do
    {
        ...
        snprintf(ifcscada_ModuleInfoDesc, sizeof(ifcscada_ModuleInfoDesc), "TODO: set application specific module info string");
...
*        For general configuration data, ifcscada_CfgParams is being used.
*        Being called by ifcscada_CfgRead.
...
MLOCAL SINT32 Task_CfgRead(VOID)
{
    ...
    snprintf(section, sizeof(section), ifcscada_BaseParams.AppName);
    ...
    for (idx = 0; idx < NbOfTasks; idx++)
    {
        ...
        ret = pf_GetStrg(section, group, key, "", (CHAR *) & TmpStrg, sizeof(TmpStrg),
                         ifcscada_BaseParams.CfgLine, ifcscada_BaseParams.CfgFileName);
        ...
        else
        {
            ...
            {
                TaskList[idx]->Priority = ifcscada_BaseParams.DefaultPriority;
...
MLOCAL SINT32 Task_CreateAll(VOID)
{
    ...
    for (idx = 0; idx < NbOfTasks; idx++)
    {
        ...
        if (TaskList[idx]->WDogRatio > 0)
        {
            ...
            TaskList[idx]->WdogId = sys_WdogCreate(ifcscada_AppName, wdogtime_us);
        ...
        if (strlen(TaskList[idx]->Name) < 1)
            snprintf(TaskList[idx]->Name, sizeof(TaskList[idx]->Name), "a%s_%d", ifcscada_AppName, idx + 1);
        ...
        TaskList[idx]->TaskId = sys_TaskSpawn(ifcscada_AppName, TaskName,
...
MLOCAL VOID Task_DeleteAll(VOID)
{
    ...
    do
    {
        ...
        if (AllTasksQuitted)
        {
            if (ifcscada_BaseParams.DebugMode & APP_DBG_INFO1)
...
MLOCAL SINT32 Task_InitTiming_Sync(TASK_PROPERTIES * pTaskData)
{
    ...
    pTaskData->SyncSessionId = mio_StartSyncSession(ifcscada_AppName);
...
MLOCAL VOID Task_WaitCycle(TASK_PROPERTIES * pTaskData)
{
     ...
     */
    if ((ifcscada_ModState == RES_S_STOP) || (ifcscada_ModState == RES_S_EOI))
    {
         ...
         */
        semTake(ifcscada_StateSema, WAIT_FOREVER);
    }
}
...
MLOCAL VOID ifcscada_CfgInit(VOID)
{
    ...
    strncpy(ifcscada_BaseParams.CfgFileName, ifcscada_ProfileName, M_PATHLEN);

    /* Application name */
    strncpy(ifcscada_BaseParams.AppName, ifcscada_AppName, M_MODNAMELEN);

    /* Line number in configuration file (used as start line for searching) */
    ifcscada_BaseParams.CfgLine = ifcscada_CfgLine;

    /* Worker task priority from module base parameters (BaseParms) */
    ifcscada_BaseParams.DefaultPriority = ifcscada_AppPrio;

    /* Debug mode from module base parameters (BaseParms) */
    ifcscada_BaseParams.DebugMode = ifcscada_Debug;
...
SINT32 ifcscada_SviSrvInit(VOID)
{
    ...
    UINT32  i;
    CHAR    Func[] = "ifcscada_SviSrvInit";

    /* If there are any SVI variables to be exported */
    if (NbOfGlobVars)
    {
        /* Initialize SVI-handler */
        ifcscada_SviHandle = svi_Init(ifcscada_AppName, 0, 0);
        if (!ifcscada_SviHandle)
        {
    ...
    {
        ret = svi_AddGlobVar(ifcscada_SviHandle, SviGlobVarList[i].VarName,
...
/**
********************************************************************************
* @brief Frees SVI server resources according to ifcscada_SviSrvInit()
...
VOID ifcscada_SviSrvDeinit(VOID)
{
    ...
    if (svi_DeInit(ifcscada_SviHandle) < 0)
        LOG_E(0, "ifcscada_SviSrvDeinit", "Could not de-initialize SVI server");
