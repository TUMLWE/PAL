...
/* Project includes */
#include "wfstatus.h"
#include "wfstatus_e.h"
#include "wfstatus_int.h"
...
SINT32  wfstatus_AppEOI(VOID);
VOID    wfstatus_AppDeinit(VOID);
SINT32  wfstatus_CfgRead(VOID);
SINT32  wfstatus_SviSrvInit(VOID);
VOID    wfstatus_SviSrvDeinit(VOID);

/* Functions: administration, to be called only from within this file */
MLOCAL VOID wfstatus_CfgInit(VOID);
...
WFSTATUS_BASE_PARMS wfstatus_BaseParams;
...
UINT16 host_scada_WT1TurbineState[1];
MLOCAL SVI_ADDR SA_host_scada_WT1TurbineState;
UINT16 host_scada_WT2TurbineState[1];
MLOCAL SVI_ADDR SA_host_scada_WT2TurbineState;
UINT16 host_scada_WT3TurbineState[1];
MLOCAL SVI_ADDR SA_host_scada_WT3TurbineState;
...
REAL64 host_scada_checkisWFon[1];
MLOCAL SVI_ADDR SA_host_scada_checkisWFon;
REAL64 host_scada_checkWindInSector[1];
MLOCAL SVI_ADDR SA_host_scada_checkWindInSector;
REAL64 host_scada_checkall[1];
MLOCAL SVI_ADDR SA_host_scada_checkall;
...
MLOCAL REAL64 wt1_status=0;
MLOCAL REAL64 wt2_status=0;
MLOCAL REAL64 wt3_status=0;
MLOCAL REAL64 avg_ws=0;
MLOCAL REAL64 avg_wd=0;
MLOCAL BOOL8 check_isWFon=0;
MLOCAL BOOL8 check_WindInSector=0;
MLOCAL BOOL8 check_all=0;
...
MLOCAL TASK_PROPERTIES TaskProperties_aControl = {
    "aWFSTATUS_Ctrl",                 /* unique task name, maximum length 14 */
...
MLOCAL SVI_GLOBVAR SviGlobVarList[] = {
    {"CycleCounter", SVI_F_INOUT | SVI_F_UINT32, sizeof(UINT32), (UINT32 *) & CycleCount, 0, NULL,
     NULL}
    ,
    {"ModuleVersion", SVI_F_OUT | SVI_F_STRING, sizeof(wfstatus_Version),
     (UINT32 *) wfstatus_Version, 0, NULL, NULL},

// MATLABCODEGEN: OpenField SVI Variables Coupling

{"wt1_status", SVI_F_INOUT | SVI_F_BLK | SVI_F_REAL64, sizeof(REAL64[1]), (UINT32 *) &wt1_status, 0, NULL, NULL},
{"wt2_status", SVI_F_INOUT | SVI_F_BLK | SVI_F_REAL64, sizeof(REAL64[1]), (UINT32 *) &wt2_status, 0, NULL, NULL},
{"wt3_status", SVI_F_INOUT | SVI_F_BLK | SVI_F_REAL64, sizeof(REAL64[1]), (UINT32 *) &wt3_status, 0, NULL, NULL},
{"avg_ws", SVI_F_INOUT | SVI_F_BLK | SVI_F_REAL64, sizeof(REAL64[1]), (UINT32 *) &avg_ws, 0, NULL, NULL},
{"avg_wd", SVI_F_INOUT | SVI_F_BLK | SVI_F_REAL64, sizeof(REAL64[1]), (UINT32 *) &avg_wd, 0, NULL, NULL},
{"check_isWFon", SVI_F_INOUT | SVI_F_BOOL8, sizeof(BOOL8[1]), (UINT32 *) &check_isWFon, 0, NULL, NULL},
{"check_WindInSector", SVI_F_INOUT | SVI_F_BOOL8, sizeof(BOOL8[1]), (UINT32 *) &check_WindInSector, 0, NULL, NULL},
{"check_all", SVI_F_INOUT | SVI_F_BOOL8, sizeof(BOOL8[1]), (UINT32 *) &check_all, 0, NULL, NULL},
...
wt1_status = host_scada_WT1TurbineState[0];
wt2_status = host_scada_WT2TurbineState[0];
wt3_status = host_scada_WT3TurbineState[0];
avg_ws = host_scada_avgws110m[0];
avg_wd = host_scada_avgwd110m[0];
...
WF_status_U.wt1_status =  wt1_status;
WF_status_U.wt2_status =  wt2_status;
WF_status_U.wt3_status =  wt3_status;
WF_status_U.avg_ws =  avg_ws;
WF_status_U.avg_wd =  avg_wd;
...
check_isWFon =  WF_status_Y.check_isWFon;
check_WindInSector =  WF_status_Y.check_WindInSector;
check_all =  WF_status_Y.check_all;

// MATLABCODEGEN: CloseField Output Variable Assignment

// MATLABCODEGEN: OpenField Assign ITF output Variables to HOST SVI

host_scada_checkisWFon[0] = check_isWFon;
host_scada_checkWindInSector[0] = check_WindInSector;
host_scada_checkall[0] = check_all;
...
SINT32 wfstatus_AppEOI(VOID)
{
    ...
    do
    {
        ...
        snprintf(wfstatus_ModuleInfoDesc, sizeof(wfstatus_ModuleInfoDesc), "TODO: set application specific module info string");
...
*        For general configuration data, wfstatus_CfgParams is being used.
*        Being called by wfstatus_CfgRead.
...
MLOCAL SINT32 Task_CfgRead(VOID)
{
    ...
    snprintf(section, sizeof(section), wfstatus_BaseParams.AppName);
    ...
    for (idx = 0; idx < NbOfTasks; idx++)
    {
        ...
        ret = pf_GetStrg(section, group, key, "", (CHAR *) & TmpStrg, sizeof(TmpStrg),
                         wfstatus_BaseParams.CfgLine, wfstatus_BaseParams.CfgFileName);
        ...
        else
        {
            ...
            {
                TaskList[idx]->Priority = wfstatus_BaseParams.DefaultPriority;
...
MLOCAL SINT32 Task_CreateAll(VOID)
{
    ...
    for (idx = 0; idx < NbOfTasks; idx++)
    {
        ...
        if (TaskList[idx]->WDogRatio > 0)
        {
            ...
            TaskList[idx]->WdogId = sys_WdogCreate(wfstatus_AppName, wdogtime_us);
        ...
        if (strlen(TaskList[idx]->Name) < 1)
            snprintf(TaskList[idx]->Name, sizeof(TaskList[idx]->Name), "a%s_%d", wfstatus_AppName, idx + 1);
        ...
        TaskList[idx]->TaskId = sys_TaskSpawn(wfstatus_AppName, TaskName,
...
MLOCAL VOID Task_DeleteAll(VOID)
{
    ...
    do
    {
        ...
        if (AllTasksQuitted)
        {
            if (wfstatus_BaseParams.DebugMode & APP_DBG_INFO1)
...
MLOCAL SINT32 Task_InitTiming_Sync(TASK_PROPERTIES * pTaskData)
{
    ...
    pTaskData->SyncSessionId = mio_StartSyncSession(wfstatus_AppName);
...
MLOCAL VOID Task_WaitCycle(TASK_PROPERTIES * pTaskData)
{
     ...
     */
    if ((wfstatus_ModState == RES_S_STOP) || (wfstatus_ModState == RES_S_EOI))
    {
         ...
         */
        semTake(wfstatus_StateSema, WAIT_FOREVER);
    }
}
...
MLOCAL VOID wfstatus_CfgInit(VOID)
{
    ...
    strncpy(wfstatus_BaseParams.CfgFileName, wfstatus_ProfileName, M_PATHLEN);

    /* Application name */
    strncpy(wfstatus_BaseParams.AppName, wfstatus_AppName, M_MODNAMELEN);

    /* Line number in configuration file (used as start line for searching) */
    wfstatus_BaseParams.CfgLine = wfstatus_CfgLine;

    /* Worker task priority from module base parameters (BaseParms) */
    wfstatus_BaseParams.DefaultPriority = wfstatus_AppPrio;

    /* Debug mode from module base parameters (BaseParms) */
    wfstatus_BaseParams.DebugMode = wfstatus_Debug;
...
SINT32 wfstatus_SviSrvInit(VOID)
{
    ...
    UINT32  i;
    CHAR    Func[] = "wfstatus_SviSrvInit";

    /* If there are any SVI variables to be exported */
    if (NbOfGlobVars)
    {
        /* Initialize SVI-handler */
        wfstatus_SviHandle = svi_Init(wfstatus_AppName, 0, 0);
        if (!wfstatus_SviHandle)
        {
    ...
    {
        ret = svi_AddGlobVar(wfstatus_SviHandle, SviGlobVarList[i].VarName,
...
/**
********************************************************************************
* @brief Frees SVI server resources according to wfstatus_SviSrvInit()
...
VOID wfstatus_SviSrvDeinit(VOID)
{
    ...
    if (svi_DeInit(wfstatus_SviHandle) < 0)
        LOG_E(0, "wfstatus_SviSrvDeinit", "Could not de-initialize SVI server");
...
if (svi_GetAddr(pSviLib_hstscada, "WT1_TurbineState", &SA_host_scada_WT1TurbineState, &SviFormat) != SVI_E_OK)
{
    LOG_W(0, Func, "Could not get address of value host_scada_WT1TurbineState!");
    return (ERROR);
}
if (svi_GetAddr(pSviLib_hstscada, "WT2_TurbineState", &SA_host_scada_WT2TurbineState, &SviFormat) != SVI_E_OK)
{
    LOG_W(0, Func, "Could not get address of value host_scada_WT2TurbineState!");
    return (ERROR);
}
if (svi_GetAddr(pSviLib_hstscada, "WT3_TurbineState", &SA_host_scada_WT3TurbineState, &SviFormat) != SVI_E_OK)
{
    LOG_W(0, Func, "Could not get address of value host_scada_WT3TurbineState!");
    return (ERROR);
}
...
if (svi_GetAddr(pSviLib_hstscada, "check_isWFon", &SA_host_scada_checkisWFon, &SviFormat) != SVI_E_OK)
{
    LOG_W(0, Func, "Could not get address of value host_scada_checkisWFon!");
    return (ERROR);
}
if (svi_GetAddr(pSviLib_hstscada, "check_WindInSector", &SA_host_scada_checkWindInSector, &SviFormat) != SVI_E_OK)
{
    LOG_W(0, Func, "Could not get address of value host_scada_checkWindInSector!");
    return (ERROR);
}
if (svi_GetAddr(pSviLib_hstscada, "check_all", &SA_host_scada_checkall, &SviFormat) != SVI_E_OK)
{
    LOG_W(0, Func, "Could not get address of value host_scada_checkall!");
    return (ERROR);
}

// MATLABCODEGEN: CloseField Get ITF SA Address

if (svi_GetAddr(pSviLib_hstscada, "WF_status_AppStatus", &SA_AppStatus, &SviFormat) != SVI_E_OK)
{
...
UINT32 host_scada_WT1TurbineState_size = sizeof(host_scada_WT1TurbineState);
UINT32 host_scada_WT2TurbineState_size = sizeof(host_scada_WT2TurbineState);
UINT32 host_scada_WT3TurbineState_size = sizeof(host_scada_WT3TurbineState);
UINT32 host_scada_avgws110m_size = sizeof(host_scada_avgws110m);
UINT32 host_scada_avgwd110m_size = sizeof(host_scada_avgwd110m);
...
ret = svi_GetBlk(pSviLib_hstscada, SA_host_scada_WT1TurbineState, (UINT32*) &host_scada_WT1TurbineState,  &host_scada_WT1TurbineState_size);
if (ret != SVI_E_OK)
   LOG_W(0, "SviClt_Read", "Could not read value host_scada_WT1TurbineState!");
ret = svi_GetBlk(pSviLib_hstscada, SA_host_scada_WT2TurbineState, (UINT32*) &host_scada_WT2TurbineState,  &host_scada_WT2TurbineState_size);
if (ret != SVI_E_OK)
   LOG_W(0, "SviClt_Read", "Could not read value host_scada_WT2TurbineState!");
ret = svi_GetBlk(pSviLib_hstscada, SA_host_scada_WT3TurbineState, (UINT32*) &host_scada_WT3TurbineState,  &host_scada_WT3TurbineState_size);
if (ret != SVI_E_OK)
   LOG_W(0, "SviClt_Read", "Could not read value host_scada_WT3TurbineState!");
ret = svi_GetBlk(pSviLib_hstscada, SA_host_scada_avgws110m, (UINT32*) &host_scada_avgws110m,  &host_scada_avgws110m_size);
if (ret != SVI_E_OK)
   LOG_W(0, "SviClt_Read", "Could not read value host_scada_avgws110m!");
ret = svi_GetBlk(pSviLib_hstscada, SA_host_scada_avgwd110m, (UINT32*) &host_scada_avgwd110m,  &host_scada_avgwd110m_size);
if (ret != SVI_E_OK)
   LOG_W(0, "SviClt_Read", "Could not read value host_scada_avgwd110m!");
...
ret = svi_SetVal(pSviLib_hstscada, SA_host_scada_checkisWFon, *(UINT32*) &host_scada_checkisWFon );
if (ret != SVI_E_OK)
     LOG_W(0, "SviClt_Write", "Could not read value host_scada_checkisWFon!");

ret = svi_SetVal(pSviLib_hstscada, SA_host_scada_checkWindInSector, *(UINT32*) &host_scada_checkWindInSector );
if (ret != SVI_E_OK)
     LOG_W(0, "SviClt_Write", "Could not read value host_scada_checkWindInSector!");

ret = svi_SetVal(pSviLib_hstscada, SA_host_scada_checkall, *(UINT32*) &host_scada_checkall );
if (ret != SVI_E_OK)
     LOG_W(0, "SviClt_Write", "Could not read value host_scada_checkall!");
